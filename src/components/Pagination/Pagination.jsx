...
export const Pagination = ({ pagination }) => {
  ...
  const shownProducts = Math.min(limit * currentPage, totalProducts);

  const location = useLocation();
  const [searchParam] = useSearchParams();

  const createPageUrl = (currentPage) => {
  ...
  const nextPageUrl =
    nextPageNumber <= totalPages ? createPageUrl(currentPage + 1) : "";
    ...
    <div className={s.pagination}>
      <div className={s.bar}>
        <div
          ...
          style={{
            width: `calc(${shownProducts} / ${totalProducts} * 100%)`,
          }}></div>
